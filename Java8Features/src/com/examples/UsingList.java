...
public class UsingList {
	public static void main(String[] args) {
		...
		List v = list1.stream().distinct().collect(Collectors.toList());
		List v1 = list1.stream().sorted().collect(Collectors.toList());
		System.out.println("Sorted order : " + v1);
		System.out.println("Unique values : " + v);
		int add = list1.stream().reduce(0, (a, b) -> a + b);
		...
		List<Integer> decreasing = list.stream().sorted(Comparator.reverseOrder()).distinct()
		...
		Integer Thirdhighest = list.stream().sorted(Comparator.reverseOrder()).distinct().skip(2).findFirst()
				.orElse(-1);
		System.out.println("ThirdHighest " + Thirdhighest);

		List<String> contains = list.stream().map(j -> String.valueOf(j)).filter(f -> f.contains("2"))
				.collect(Collectors.toList());
		System.out.println("Value contains from list are " + contains);

		List<String> startswith = list.stream().map(g -> String.valueOf(g)).filter(n -> n.contains("5"))
				.collect(Collectors.toList());
		System.out.println("startswith " + startswith);
		List<Integer> twoLists = Stream.concat(list.stream(), list1.stream()).sorted(Comparator.reverseOrder())
		...
		List<Integer> limit_Skip = list.stream().limit(5).skip(1).filter(d -> d % 2 == 0).map(x -> x * x)
				.collect(Collectors.toList());
		System.out.println("limit_Skip" + limit_Skip);
		List<Integer> limit_Sk = list.stream().skip(1).limit(2).filter(d -> d % 2 == 0).map(x -> x * x)
				.collect(Collectors.toList());
